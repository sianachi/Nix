...
`mise install` pins the toolchain (Node 22, pnpm 10, .NET 10, Go 1.26). Run frontend commands from the repository root.
...
Inspect `.github/workflows/` for current CI path filters, including backend, frontend and worker checks.
Use `scripts/changed-path-checks.sh` to select the local minimum; it does not replace CI.
...
2. **Then close the gap in the same session**: extend the CLI _and_ the MCP so that class of problem
...
- **Never trust a remembered ADR inventory.** List `docs/adr/` immediately before assigning or citing a number, verify that the referenced file exists, and do not fill an apparent historical gap without owner direction. The current worker topology decision is ADR-0048 (superseding ADR-0046 for topology) and personal-workspace/JIT policy is ADR-0045.
