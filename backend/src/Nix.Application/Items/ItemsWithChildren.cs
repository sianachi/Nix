...
/// </para>
/// </remarks>
public sealed class ItemsWithChildren
{
    private readonly IItemTree _tree;

    /// <summary>Initializes a new instance of the <see cref="ItemsWithChildren"/> class.</summary>
    /// <param name="tree">Item storage.</param>
    public ItemsWithChildren(IItemTree tree)
    {
    ...
    public async ValueTask<IReadOnlySet<ItemId>> ExecuteAsync(
        WorkspaceId workspaceId,
        IReadOnlyList<ItemId> parents,
        CancellationToken cancellationToken) =>
        await _tree.WithChildrenAsync(workspaceId, parents, cancellationToken).ConfigureAwait(false);
}
