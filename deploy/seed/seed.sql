...
\if :{?collab_password} \else \set collab_password 'nix-dev-collab' \endif
...
BEGIN
    ...
    -- The collaboration service. A third role rather than sharing nix_app, because the two have
    -- deliberately different privileges: content tables are read-write here and read-only for the
    -- API. Sharing a role would make that split unenforceable.
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'nix_collab') THEN
        CREATE ROLE nix_collab LOGIN;
        RAISE NOTICE 'created role nix_collab';
    ELSE
        RAISE NOTICE 'role nix_collab already present';
    END IF;
END
$$;
...
ALTER ROLE nix_collab WITH PASSWORD :'collab_password';
...
ALTER ROLE nix_collab WITH NOBYPASSRLS NOSUPERUSER NOCREATEDB NOCREATEROLE;
...
BEGIN
    ...
    END IF;
    IF (SELECT rolbypassrls FROM pg_roles WHERE rolname = 'nix_collab') THEN
        RAISE EXCEPTION
            'nix_collab has BYPASSRLS; row-level security would not isolate tenants';
    END IF;
    RAISE NOTICE 'verified nix_app and nix_collab cannot bypass row-level security';
END
$$;
